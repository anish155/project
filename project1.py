...
driver_data=[]# list to store data from the files
racer_info={}# dictionary to store data fromn f1_driver.txt

def racer_data(file):
     #to store racer name and team from f1_racer.txt
...
def lap_data_store(lap_file,lap_no):
     # to store racer code, full name, team, spped in driver data list as list of dictionaries
...
def race_name(lap_file, lap_no):
     # reads first line from every lap file and print the lap name.
    with open(lap_file, 'r') as f:
        location = f.readline().strip()
        print(f"Lap {lap_no} race is in {location}.")

def fastest_racer(lap_no):
     #to compare and diaplay fastest recer and there speed in every lap
     ...
     print(f"Driver: {fastest['full name']} ({fastest['driver']}), team:{fastest['team']}, speed: {fastest['car speed']:.3f}")
    
def fast_individual_racer():
     # to display fast spped of every racer in every lap using table
...
def fast_individual_racer_descending():
     #to display fast speed of every racer in every lap but in descending order 
...
def average_speed_racers():
     #to calculate average speed from every racer and display
...
def average_speed_individual():
     #to calculate and show average speed of all racers in table
     racer_average={}
     ...
     for entry in driver_data:
          ...
          if driver not in racer_average:
               racer_average[driver] ={"total time":0, "count":0, "full name": entry["full name"], "team": entry["team"]}
...
def plot_fastest_racers():
     #to plot graph on fastest speed of racers
     individual_drive_speed={}
...
def plot_average_speed():
     #to plot graph on average speed of racers
     racer_average={}
...
if len(sys.argv)<2:#for command line 
...
try:
     ...
     for i in range(1,4):
          lap_data_store(sys.argv[1].replace("X", str(i)), i)
